...
try {
  if (-not $Msix) {
    ...
    Invoke-Checked bare-build @('--config', './bare-build.config.cjs', '--host', 'win32-x64', '--out', $out, 'index.js')

    $exe = Join-Path $repo "out\win32-x64\$name.exe"
    Sign-File -Path $exe -Certificate $cert -Subject $Subject

    Write-Host "Signed executable: $exe"
    return
  }
  ...
  try {
    ...
    Invoke-Checked bare-build @('--config', './bare-build.msix.config.cjs', '--host', 'win32-x64', '--out', './out/win32-x64-msix-dir', 'index.js')
