...
// libuv's UV_TTY_MODE_RAW_VT. setRawMode(true) maps to UV_TTY_MODE_RAW, which
// on Windows sets only ENABLE_WINDOW_INPUT: libuv translates keyboard records to
// VT sequences but *drops* mouse INPUT_RECORDs, so mouse reports never reach us.
// RAW_VT additionally enables the console's ENABLE_VIRTUAL_TERMINAL_INPUT, so the
// console itself emits mouse reports as VT bytes that flow through libuv's
// KEY_EVENT path to our MouseParser. Unix terminals already deliver mouse bytes
// in plain raw mode, so this is only needed on Windows.
...
module.exports = class Program {
  ...
  _setup() {
    if (this.input) {
      if (this.inputIsTTY && this.input.setRawMode) this._enterRawMode()
  ...
  // Put the input TTY into raw mode. Mouse tracking on Windows needs the
  // console's virtual-terminal input mode (RAW_VT) or mouse reports are dropped
  // by libuv before they reach JS; fall back to plain raw mode if this libuv
  // build won't accept it (or we don't need it).
  _enterRawMode() {
    if (this._mouseMode && global.Bare.platform === 'win32' && this.input.setMode) {
      try {
        this.input.setMode(TTY_MODE_RAW_VT)
        return
      } catch {}
    }
