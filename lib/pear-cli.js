...
const { createLogger, bridgeWorkerRun } = require('./cli-bridge')
...
function createPearCli(pkg, opts = {}) {
  ...
  // All wrapper output goes through this so TUI mode can redirect it off the
  // screen. Defaults to console only once a mode (start/tui) is chosen.
  ...
  logger.log(`${appName} v${pkg.version}`)
  logger.log(`Updates: ${updates === false ? 'disabled' : 'enabled'}`)
  ...
  pear.on('error', (err) => {
    ...
    else logger.error('[pear-runtime:error]', err)
  })
  ...
  // Keep handler refs so TUI mode can hand signal control to the Program (which
  // must restore the terminal *before* we exit the process).
  const signalHandlers = []
  function installSignals() {
    for (const sig of ['SIGINT', 'SIGTERM', 'SIGHUP', 'SIGQUIT']) {
      const handler = () => teardown(0)
  ...
  // Classic mode: run a one-shot setup handler and log to the console.
  function start(handler) {
    logger.toConsole()
    handler({ run: runWorker, flags, ...ctx })
    logger.log('CLI ready. Press Ctrl+C to stop.')
  }

  // TUI mode: drive a tea model as the UI.
  //   build({ run, send, flags, ...ctx }) => model
  // We hand signal handling to the Program (so the terminal is restored before
  // the process exits), route wrapper/worker output into the model as Msgs, and
  // close the pear runtime once the model quits.
  function tui(build, programOpts = {}) {
    removeSignals()
    ...
    const run = bridgeWorkerRun(runWorker, send)
    const model = build({ run, send, flags, ...ctx })

    program = new Program(model, programOpts)
    logger.attach((level, line) => send({ type: 'log', level, line }))

    const done = program.run().then(() => teardown(0))
    return { program, send, done }
  }

  return { start, tui, run: runWorker, teardown, logger, flags, ...ctx }
}
